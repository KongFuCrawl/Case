...
class GuaziSpider(scrapy.Spider):
   ...
   # 2 重写start_requests()方法
    def start_requests(self):
        """一次性生成所有的要抓取的URL地址，一次性交给调度器入队列"""
        for i in range(1, 6):
            url = 'https://www.guazi.com/langfang/buy/o{}/bread'.format(i)
            #交给调度器入队列，并指定解析函数
            yield scrapy.Request(url=url, callback=self.parse.datail_page)



    def detail_page(self, response):

        li_list = response.xpath('//ul[contains(@class, "list")]/li')
        for li in li_list:
            # 给item.py中的GuaziItem类实例化
            item = GuaziItem()
