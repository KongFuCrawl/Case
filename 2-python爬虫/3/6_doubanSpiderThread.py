"""
    ...
    3,查看QueryString Parameters 的规律
...
class DoubanSpider:
    def __init__(self):
        self.url = 'https://movie.douban.com/j/chart/top_list?type=11&interval_id=100:90&action=&start={}&limit=20'
        # 创建队列
        self.q = Queue()
        # 创建锁
        self.lock = Lock()
        ...
        while True:
            ...
            if not self.q.empty():
                ...
                for one_film_dict in html:
                    ...
                    item['url'] = one_film_dict['rank']
                    item['score'] = one_film_dict['score']
                    item['tiem'] = one_film_dict['release_date']
                    print(item)
            else:
                self.lock.release()
                break
